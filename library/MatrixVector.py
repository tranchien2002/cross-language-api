...
from library.document2json import pickle_json, cosine_json
...
def store_matrix():
    ...
    ouf = open('library/pickle_en.txt', 'wb')
    cPickle.dump(documents_en, ouf)
    ouf.close()

def get_matrix_vectors():
    inf = open('library/pickle_en.txt', 'rb')
...
#test
def find_docs(object, index):
    doc = docs_col.find_one({"_id": ObjectId(str(object[index]['id']))})
    print(doc['title'])
    print("-----------------------------------------------------")
    print(doc['content'])
