...
from library.Translator import translate, translate_yandex
...
def counter(lang, word = False):
    articles = mongo_col.find({"lang": lang})

    if(word):
        contents = [item['words'] for item in articles]
    else:
...
def get_stopwords(language):
    if(language == "vietnamese"):
        with open("library/stopwords_vn.txt") as f:
            content = f.readlines()
...
def extract_topn_from_vector(doc, table , topn=15, word = False):
...
def extract_topn_new_vi_doc(content, topn = 15):
    content = ViTokenizer.tokenize(convert_text(content))
...
def update_keyword(id, new_keyword, lang = ""):
    ...
    mongo_col.update({'_id': ObjectId(id)}, {'$set' : {'keyword': new_keyword}})
...
if __name__ == '__main__':
    ...
    for a in articles_vi:
        update_keyword(str(a['_id']), extract_topn_from_vector(a['words'], "vietnamese", 15, word=True), lang="vietnamese")
